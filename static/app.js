...
async function saveSettings() {const data=Object.fromEntries(new FormData($('#settings-form')));for(const k of ['max_steps','max_searches','max_pages','max_output_tokens','timeout_seconds'])data[k]=Number(data[k]);data.profile_to_model=$('[name=profile_to_model]').checked;for(const key of ['api_key','search_key']){if(data['clear_'+key])data[key]='';else if(!data[key])delete data[key];delete data['clear_'+key];}await api('/settings',{method:'PUT',body:JSON.stringify(data)});await refresh();}
